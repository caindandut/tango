        ...
        {day.grammarPoints.map((point) => (
          <article key={point.id} className="rounded-2xl border border-white/10 bg-slate-900/60 p-5 sm:p-6">
            ...
            <div className="mt-4 space-y-3">
              ...
              <div><p className="text-[10px] font-bold uppercase tracking-wider text-accent-quizlet">Cấu trúc</p><ul className="mt-1 space-y-1 text-sm text-primary-100" lang="ja">{point.structures.map((structure) => <li key={structure} className="rounded-lg bg-white/[0.04] px-3 py-2">{structure}</li>)}</ul></div>
